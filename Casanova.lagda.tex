...
\AgdaFunction{uniquePrepend} indicates that if \AgdaBound{x} is not in \AgdaBound{xs}, then \AgdaFunction{pos} \AgdaSymbol(\AgdaBound{x} \AgdaOperator{\AgdaFunction{List.∷}} \AgdaBound{xs}\AgdaSymbol) contains the successor of \AgdaFunction{pos} \AgdaBound{xs} or is \AgdaInductiveConstructor{nothing}.

\begin{code}
    unique-prepend :
      (x : A) ->
      (xs : List.List A) ->
      AllList.All (\ e -> x ≢ e) xs ->
      pos (x List.∷ xs) ≡ Data.Maybe.map Fin.suc (pos xs)
    unique-prepend x List.[] all = refl
    unique-prepend x xs@(x1 List.∷ xs1) alls@(a AllList.∷ as) = begin
      pos (x List.∷ xs)
        ≡⟨⟩
      Data.Maybe.map Fin.suc (posh List.[ x ] xs)
        ≡⟨⟩
      Data.Maybe.map Fin.suc
        (if isYes (any? (DEQ x1) List.[ x ])
            then just Fin.zero
            else Data.Maybe.map Fin.suc (posh (x1 List.∷ List.[ x ]) xs1))
        ≡⟨ cong (\ e -> Data.Maybe.map Fin.suc
                          (if isYes e
                              then just Fin.zero
                              else Data.Maybe.map Fin.suc (posh (x1 List.∷ List.[ x ]) xs1)))
                x1-is-decidably-not-in-[x] ⟩
      Data.Maybe.map Fin.suc (Data.Maybe.map Fin.suc (posh (x1 List.∷ List.[ x ]) xs1))
        ≡⟨ cong (Data.Maybe.map Fin.suc) (sym (posh-moves-elements {x} {x1} {xs1} )) ⟩
      Data.Maybe.map Fin.suc (posh List.[ x ] xs)
        ≡⟨ cong (Data.Maybe.map Fin.suc) (sym (pos-is-posh {x} {xs} alls)) ⟩
      Data.Maybe.map Fin.suc (posh List.[] xs) ∎
      where
      posh-moves-elements :
        {x y : A} ->
        {xs : List.List A} ->
...
\begin{code}
    ...
    ... | no bogus = contradiction isNothing notNothing
      where
      nonnothing-is-just :
        {a : Level} ->
        {A : Set a} ->
      ...
      notNothing = just-is-not-nothing (nonunique-prepend x (y List.∷ ys) (nonnothing-is-just bogus))
\end{code}
