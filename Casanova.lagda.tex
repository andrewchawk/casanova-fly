...
\AgdaFunction{CAS.factor} \AgdaBound{c} \AgdaBound{n} \AgdaBound{e} is a list of all nontrivial \AgdaBound{n}-vectors whose products are \AgdaBound{c}-\gls{injEq} to \AgdaBound{e}.
...
record CAS {a : Level} (ε : Set a) : Set (Level.suc a) where
  field
    ...
    factor : (n : ℕ) → ε → List.List (Vec ε n)
...
\AgdaField{VCAS.factor-input-is-product-of-output} \AgdaBound{v} proves that for every value \(x\) in \AgdaField{CAS.factor} \AgdaBound{c} \AgdaBound{n} \AgdaBound{e}, the product of \(x\) is \gls{algEq} to \AgdaBound{e}.
...
record VCAS {a : Level} {ε : Set a} (Cas : CAS ε) : Set a where
  ...
  field
    ...
    factor-input-is-product-of-output :
      (n : ℕ) →
      (e : ε) →
      AllList.All (λ x → e ≈ Data.Vec.foldr _ _*_ (fromℕ 1) x) (factor n e)
