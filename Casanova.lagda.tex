  ...
  module pfRewrite-basicEquality where
    ...
    specific-equality g f x eq = begin
      pfRewrite (Ap g (Data.Vec.cast (sym eq) [ Ap f x ]))
        ≡⟨ refl ⟩
      _>>=_ (decToMaybe (arity g Data.Nat.≟ 1))
            (λ eq → helper g
                           (onlyElement (Data.Vec.cast (sym eq)
                                                       [ Ap f x ])
                                        eq)
                           eq)
        ≡⟨ cong (Data.Maybe._>>= λ eq →
                                   helper g
                                          (onlyElement (Data.Vec.cast (sym eq)
                                                                      [ Ap f x ])
                                                       eq)
      ...
      _>>=_ (just eq)
            (λ eq →
               helper g (onlyElement (Data.Vec.cast (sym eq)
                                                    [ Ap f x ])
                                     eq)
                        eq)
        ≡⟨⟩
    ...
    ... | Ap f y ∷ [] | ⟦ eq ⟧ =
          Transymmetry (subst (λ e → AlgebraicEquality (Ap (Compose g f rightArity) y)
                       ...
                       (EqualityImpliesAE (sym (begin
                         to-witness isJust
                           ≡⟨ {!!} ⟩
                         Ap (Compose g f rightArity) y ∎)))
          where
