...
\begin{itemize}
  \item If applicable, the \(\left(\AgdaFunction{firstRepeatPosition} \AgdaBound{DEQ} \AgdaBound{x}\right)\)th position of \AgdaBound{x} is the position of the first repeating value in \AgdaBound{x}.
  \item If \AgdaBound{x} has no repeating values, then \AgdaFunction{firstRepeatPosition} \AgdaBound{DEQ} \AgdaBound{x} is just the length of \AgdaBound{x}.
\end{itemize}
...
module firstRepeatPosition
  ...
  firstRepeatPosition : (x : List.List A) -> Fin (ℕ.suc (List.length x))
  firstRepeatPositionHelper : (x y : List.List A) -> Fin (ℕ.suc (List.length y))

  firstRepeatPosition = firstRepeatPositionHelper List.[]

  firstRepeatPositionHelper x List.[] = Fin.zero
  firstRepeatPositionHelper List.[] (y List.∷ ys) =
    Fin.suc (firstRepeatPositionHelper List.[ y ] ys)
  firstRepeatPositionHelper (x List.∷ xs) (y List.∷ ys) =
    if isYes (any? (DEQ y) (x List.∷ xs))
      then Fin.zero
      else Fin.suc (firstRepeatPositionHelper (y List.∷ x List.∷ xs) ys)
\end{code}
...
\subsubsection{The Proof of Equalling the Length}
\AgdaFunction{pos-of-unique-list-is-length} indicates that \AgdaFunction{firstRepeatPosition} \AgdaBound{x} is the length of \AgdaBound{x} if all elements of \AgdaBound{x} are unique.

\begin{code}
    pos-of-unique-list-is-length :
      (x : List.List A) ->
      UniqueList x ->
      pos x ≡ Fin.fromℕ (List.length x)
    pos-of-unique-list-is-length = {!!}
\end{code}

\subsubsection{The Proof of Equalling the Position of the First Repeating Value}
\AgdaFunction{pos-is-position-of-first-repeat} indicates that \AgdaFunction{firstRepeatPosition} \AgdaBound{x} is the position of the first repeating value in \AgdaBound{x} if \AgdaBound{x} has such a value.
...
nonRepeatingPortion : {a : Level} ->
                      ...
                      List.List A ->
                      List.List A
nonRepeatingPortion DEQ x = List.take (Fin.toℕ (firstRepeatPosition DEQ x)) x
\end{code}
...
record CAS {a : Level} (ε : Set a) : Set (Level.suc Level.zero Level.⊔ a) where
  ...
  traceEvaluate n e =
    nonRepeatingPortion decFun
                        (List.map (flip rEvaluate e) (List.upTo n))
      where
      decFun = Data.Sum.Properties.≡-dec Data.String._≟_
                                         structuralEquality
