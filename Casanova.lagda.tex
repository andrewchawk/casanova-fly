...
\AgdaField{VCAS.factor-input-is-product-of-output} \AgdaBound{v} proves that the \AgdaBound{v} CAS factors expressions \(e\) to expression vectors \(v\) such that the product of all elements of \(v\) is algebraically equal to \(e\).
...
record VCAS {a : Level} {ε : Set a} (Cas : CAS ε) : Set a where
  ...
  field
    ...
    factor-input-is-product-of-output :
      ...
      (e : ε) →
      (contained : Vec ε n) →
      factor n e ≡ just contained →
      _≈_ e
          (Data.Vec.foldr _ _*_ (fromℕ 1) contained)
