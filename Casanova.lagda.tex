...
\begin{code}
  ...
  main n = List.filter (λ (f1 , f2) → f1 Data.Nat.* f2 Data.Nat.≟ n)
                       (List.cartesianProduct (List.upTo (ℕ.suc n))
                                              (List.upTo (ℕ.suc n)))
\end{code}
...
\subsection{Exporting the Function}
As indicated in \fref{se:bifactorNat-main}, the author has decided to provide the ``\AgdaFunction{bifactorNat}'' alias for \AgdaFunction{bifactorNat.main}.
