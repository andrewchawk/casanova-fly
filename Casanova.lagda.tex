...
\begin{code}
    ...
    unique-element-prepend x xs@(x1 List.∷ xs1) alls@(a AllList.∷ as) = begin
      ...
      x1-is-decidably-not-in-[x] :
        Σ (¬ x1 ∈ List.[ x ])
          (λ noProof → _≡_ (any? (DEQ x1) List.[ x ])
                           (no noProof))
      x1-is-decidably-not-in-[x] =
        _,_ noProof
            (Relation.Nullary.Decidable.dec-no (any? (DEQ x1) List.[ x ])
                                               noProof)
        where noProof = AllListProp.All¬⇒¬Any (((_∘ sym) a) AllList.∷ AllList.[])
\end{code}
...
\subsection{The Sum Pattern}
As indicated by the definition, ``\AgdaBound{x} \AgdaOperator{\AgdaInductiveConstructor{C+}} \AgdaBound{y}'' is in every way equivalent to the longer ``\AgdaInductiveConstructor{Ap} \AgdaInductiveConstructor{Sum} \AgdaSymbol{(} \AgdaBound{x} \AgdaOperator{\AgdaInductiveConstructor{∷}} \AgdaBound{y} \AgdaOperator{\AgdaInductiveConstructor{∷}} \AgdaInductiveConstructor{[]} \AgdaSymbol{)}''.

\begin{code}
  pattern _C+_ x y = Ap Sum (x ∷ y ∷ [])
\end{code}

\subsection{The Negation Pattern}
\AgdaInductiveConstructor{C-\AgdaUnderscore{}} is a useful pattern for finding the additive inverse of a given expression; for all expressions \AgdaBound{e}, the additive inverse of \AgdaBound{e} exists if and only if \AgdaInductiveConstructor{C-\AgdaUnderscore{}} \AgdaBound{e} represents the additive inverse of the value which is represented by \AgdaBound{e}.

\begin{code}
  pattern C-_ x = (Ap Negate (x ∷ []))
\end{code}

\subsection{The Product Pattern}
As indicated by the following definition, \AgdaBound{x} \AgdaOperator{\AgdaInductiveConstructor{C*}} \AgdaBound{y} is in every way equivalent to the longer ``\AgdaInductiveConstructor{Ap} \AgdaInductiveConstructor{Product} \AgdaSymbol{(} \AgdaBound{x} \AgdaOperator{\AgdaInductiveConstructor{∷}} \AgdaBound{y} \AgdaOperator{\AgdaInductiveConstructor{∷}} \AgdaInductiveConstructor{[]} \AgdaSymbol{)}'', i.e., is Casanova Fly's representation of the product of the values which are represented by \AgdaBound{x} and \AgdaBound{y}.

\begin{code}
  pattern _C*_ x y = Ap Product (x ∷ y ∷ [])
\end{code}

\subsection{The Exponent Pattern}
As indicated by the following definition, \AgdaBound{x} \AgdaOperator{\AgdaInductiveConstructor{C^}} \AgdaBound{y} is in every way equivalent to the longer ``\AgdaInductiveConstructor{Ap} \AgdaInductiveConstructor{Exponent} \AgdaSymbol{(} \AgdaBound{x} \AgdaOperator{\AgdaInductiveConstructor{∷}} \AgdaBound{y} \AgdaOperator{\AgdaInductiveConstructor{∷}} \AgdaInductiveConstructor{[]} \AgdaSymbol{)}'', i.e., is Casanova Fly's representation of \(x\prime^{y\prime}\), where \(x\prime\) and \(y\prime\) are the values which are represented by \AgdaBound{x} and \AgdaBound{y}, respectively.

\begin{code}
  pattern _C^_ x y = Ap Exponent (x ∷ y ∷ [])
\end{code}
...
\begin{code}
    SumCommutes : {x y : ε} → main (x C+ y) (y C+ x)
    SumCommutes {x} {y} =
      Commutativity {Sum}
                    {x ∷ y ∷ []}
                    {y ∷ x ∷ []}
    ...
    SumIsAssociative :
      ...
      main (x1 C+ (x2 C+ x3))
           ((x1 C+ x2) C+ x3)
    SumIsAssociative = Associativity {eq = refl} refl

    ProductIsAssociative :
      {x1 x2 x3 : ε} →
      main (x1 C* (x2 C* x3))
           ((x1 C* x2) C* x3)
    ProductIsAssociative = Associativity {eq = refl} refl
\end{code}
...
\begin{code}
    rules = addRecursiveRules
      ...
      ∷ record
          ...
          ; description = nothing
          ; function =
            λ { (C- (NumberRat x)) → valid (NumberRat (ℚ.0ℚ ℚ.- x))
              ; _ → nothing}
          }
      ∷ record
          { name = "0 + x = x"
      ...
      ∷ record
          ...
          ; description = nothing
          ; function =
            λ { (NumberNatP x c1 C^ NumberNatP y c2) →
      ...
      ∷ record
          ...
          ; description = nothing
          ; function =
            λ { (NumberNatP 0 coprime C^ NumberRat y) →
                if y ℚ.≤ᵇ ℚ.0ℚ
      ...
      ∷ record
          ...
          ; function =
            λ { (x C^ NumberNatP 0 c) → valid (NumberNat 1)
              ; _ → nothing}
          }
      ∷ record
          { name = "0 ^ x"
      ...
      ∷ record
          -- The structure of quotients is pretty intricate, so this rule
          -- takes precedence over the other derivative rules.
          { name = "QUOTIENT RULE FOR DERIVATIVES"
          ; description = nothing
          ; function = basicDerivativeQuotientRuleFunction
          }
      ∷ record
      ...
      ∷ record
          { name = "DERIVATIVE OF CONSTANT EXPONENT"
          ; description = nothing
          ; function =
            λ { (Ap (Derivative v1) (Ap Exponent (Variable v2 ∷ c ∷ []) ∷ [])) →
                if isYes (IsConstant? c) ∧ (v1 Data.String.== v2)
                   then valid (Ap Product
                                  ( c
                                  ∷ [ Ap Exponent
                                         ( Variable v2
                                         ∷ [ Ap Sum (c ∷ [ Ap Negate [ NumberNat 1 ] ])])]))
                   else nothing
              ; _ → nothing}
          }
      ∷ record
      ...
      ∷ record
          ...
          ; description = nothing
          ; function =
            λ { (Ap (Derivative v1) (C- x ∷ [])) →
                valid (C- (Ap (Derivative v1) (x ∷ []) ))
              ; _ → nothing}
          }
      ∷ record
          ...
          ; description = nothing
          ; function =
            λ { (Ap (Derivative v1) (x C+ y ∷ [])) →
                valid (_C+_ (Ap (Derivative v1) [ x ])
                            (Ap (Derivative v1) [ y ]))
              ; _ → nothing}
          }
      ∷ record
          ...
          ; function =
            λ { (Ap (Derivative v1) (x C* y ∷ [])) →
                valid (_C+_
                         (Ap (Derivative v1) [ x ] C* y )
                         (Ap (Derivative v1) [ y ] C* x ))
              ; _ → nothing}
          }
      ∷ record
      ...
      ∷ record
          ...
          ; function =
            λ { (Ap (Derivative v1) (Ap Cos (Variable v2 ∷ []) ∷ [])) →
                if v1 Data.String.== v2
                  then valid (C- (Ap Sin [ Variable v1 ]))
                  else nothing
              ; _ → nothing}
          }
      ∷ record
          ...
          ; function =
             λ { (Ap (Derivative v1) (Ap Tan (Variable v2 ∷ []) ∷ [])) →
                 if v1 Data.String.== v2
                   then valid (Ap Sec [ Variable v1 ] C^ NumberNat 2 )
                   else nothing
               ; _ → nothing}
          }
      ∷ record
      ...
      ∷ record
          ...
          ; description = nothing
          ; function =
            λ { (Ap (Limit n) (b ∷ (x C+ y) ∷ [])) →
                valid (Ap (Limit n) (b ∷ [ x ]) C+ Ap (Limit n) (b ∷ [ y ]))
              ; _ → nothing}
          }
      ∷ record
      ...
      ∷ record
          ...
          ; description = nothing
          ; function =
            λ { (Ap (Limit n1) (c ∷ Variable n2 C^ NumberNatP x copr ∷ [])) →
                if (n1 Data.String.== n2) ∧ (0 Data.Nat.<ᵇ x)
                   then valid (c C^ NumberNat x )
                   else nothing
              ; _ → nothing}
          }
      ∷ record
      ...
      addRecursiveRules : {n : ℕ} → Vec EvaluationRule n → List.List EvaluationRule
      addRecursiveRules list = Data.Vec.toList list List.++ List.map recurseWithRule (Data.Vec.toList list)
      ...
      basicDerivativeChainRuleFunction = b
        ...
        b _ = nothing
      basicDerivativeQuotientRuleFunction : ε → Maybe (Exceptional ε)
      basicDerivativeQuotientRuleFunction (Ap (Derivative v) (Quotient x y z ∷ [])) =
        valid (Quotient ((D x C* y) C-- (x C* D y))
                        (y C^ NumberNat 2)
                        (Coprimality.1-coprimeTo _))
        where
        _C--_ = λ x y → x C+ (C- y)
        D = Ap (Derivative v) ∘ [_]
      basicDerivativeQuotientRuleFunction x = nothing
\end{code}
    ...
    module directCheck where
      ...
      module prodCommCheck where
        applyEqualities :
          ...
          x ≡ x' →
          y ≡ y' →
          AlgebraicEquality (x C* y)
                            (y' C* x')
        applyEqualities refl refl = AlgebraicEquality.ProductCommutes

        main : Check
        main {a C* b} {c C* d} = Data.Maybe.map (uncurry applyEqualities) bi-eq
        main = nothing
...
\begin{code}
  ...
  uglyPrint (Quotient x y z) =
    Data.String.concat (Data.Vec.toList
      ...
      ∷ uglyPrint y
      ∷ [ "}" ]))
  uglyPrint (x C+ y) = uglyPrint x Data.String.++ " + " Data.String.++ uglyPrint y
  uglyPrint (x C* y) =
    Data.String.concat (Data.Vec.toList
      ( "\\left("
      ∷ uglyPrint x
        ...
        (Ap (Limit "delta")
            ...
            ∷ Quotient
                (Ap Sum ( Ap f (cast (sym arityEquality)
                                     [ Variable name C+ Variable "delta" ])
