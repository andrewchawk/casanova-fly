...
record CAS {a : Level} (ε : Set a) : Set a where
  ...
  traceEvaluate = t List.[] ∘ inj₂
    where
    t : List.List (Exceptional ε) -> Exceptional ε -> List.List (Exceptional ε)
    t xs (inj₁ x) = xs List.++ List.[ inj₁ x ]
    t xs e@(inj₂ x) with any? is-e xs
      where is-e = Data.Sum.Properties.≡-dec Data.String._≟_ structuralEquality e
    ... | yes _ = xs
    ... | no _ = t (xs List.++ List.[ inj₂ x ]) (exceptionallyEvaluate x)
...
record VCAS {a : Level} (ε : Set a) (Cas : CAS ε) : Set a where
  ...
  field
    ...
    sum-of-nats-is-nat-sum :
      (n1 n2 : ℕ) ->
      rEvaluate (sumRepresentation (fromℕ n1) (fromℕ n2)) ≡
        inj₂ (fromℕ (n1 + n2))
\end{code}
