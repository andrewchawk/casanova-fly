...
Informally, one can say that \AgdaFunction{condenseExceptionList} \AgdaBound{l} \AgdaInductiveConstructor{inj₂}-contains all values in \AgdaBound{l}, or, if applicable, \AgdaFunction{condenseExceptionList} \AgdaBound{l} \AgdaInductiveConstructor{inj₁}-contains the first exceptional value in \AgdaBound{l}.  Formally and righteously, one can say that the following statements apply:
...
\begin{itemize}
  ...
  \item \AgdaInductiveConstructor{Diff} \AgdaBound{f} represents the application function of the derivative of the \AgdaBound{f} function.  Possibly a bit more intuitively, one could say that \AgdaInductiveConstructor{Ap} \AgdaSymbol(\AgdaInductiveConstructor{Diff} \AgdaBound{f}\AgdaSymbol) \AgdaBound{x} represents the derivative of \AgdaInductiveConstructor{Ap} \AgdaBound{f} \AgdaBound{x}.
...
\begin{code}
  ...
  isCommutative (Diff name) x = false
...
\begin{code}
  ...
  structuralEqualityOnF = s
    ...
    s (Lambda n1 e1) (Diff f) = no λ ()
    s (Lambda n1 e1) (Compose g f eq) = no λ ()
    ...
    s (Antiderivative n1) (Diff f) = no λ ()
    ...
    ... | no neq = no (neq ∘ equality-implies-variable-equality n1 n2)
      ...
      equality-implies-variable-equality : (n1 n2 : String) →
             ...
             n1 ≡ n2
      equality-implies-variable-equality n1 n2 refl = refl
    s (BoundedIntegral n1) (Diff f) = no λ ()
    s (BoundedIntegral n1) (Compose g f eq) = no λ ()
    s (Diff f) Abs = no λ ()
    s (Diff f) Negate = no λ ()
    s (Diff f) Sum = no λ ()
    s (Diff f) Product = no λ ()
    s (Diff f) Exponent = no λ ()
    s (Diff f) (Lambda name exp) = no λ ()
    s (Diff f) Sin = no λ ()
    s (Diff f) Cos = no λ ()
    s (Diff f) Tan = no λ ()
    s (Diff f) Csc = no λ ()
    s (Diff f) Sec = no λ ()
    s (Diff f) Cot = no λ ()
    s (Diff f) (Limit name) = no λ ()
    s (Diff f) (Derivative name) = no λ ()
    s (Diff f) (Antiderivative name) = no λ ()
    s (Diff f) (BoundedIntegral name) = no λ ()
    s (Diff f) (Diff g) with s f g
    ... | yes equality = yes (cong Diff equality)
    ... | no different = no (different ∘ equality-implies-fun-equality)
      where
      equality-implies-fun-equality : {f g : F} → Diff f ≡ Diff g → f ≡ g
      equality-implies-fun-equality refl = refl
    s (Diff f) (Compose y x eq) = no λ ()
    ...
    s (Compose g f eq1) (Lambda name exp) = no λ ()
...
If the result is not \AgdaInductiveConstructor{nothing}, then \AgdaFunction{pfRewrite} \AgdaBound{e} contains values \AgdaBound{g} and \AgdaBound{x} such that the following statements hold:

\begin{enumerate}
  \item \AgdaBound{g} constitutes a nontrivial use of \AgdaInductiveConstructor{Compose}.\label{enum:pfRewriteCompose}
  \item \AgdaBound{e} is algebraically equal to \AgdaInductiveConstructor{Ap} \AgdaBound{g} \AgdaBound{x}.\label{enum:pfRewriteEquality}
\end{enumerate}

\begin{code}
  pfRewrite : ε → Maybe (Σ F (Vec ε ∘ arity))
  pfRewrite (Ap g x) with arity g | x | inspect arity g
  ... | 1 | Ap f y ∷ [] | ⟦ arityIs1 ⟧ = just (Compose g f arityIs1 , y)
  ... | _ | _ | _ = nothing
  pfRewrite _ = nothing
\end{code}
  ...
  module exceptionallyEvaluate where
    ...
    functions =
      ...
      ∷ (λ { (Ap Exponent ((NumberNatP 0 c) ∷ e ∷ [])) →
             ...
             if isYes (eq2 (inj₂ e) (exceptionallyEvaluate e))
                then valid (NumberNat 0)
                else [_,_] bust
                           (λ x → valid (Ap Exponent ((NumberNat 0) ∷ [ x ])))
                           (exceptionallyEvaluate e)
           ; _ → nothing})
      ...
      ∷ (λ { (Ap (Lambda (n1 List.∷ List.[]) (Ap (Derivative n2) e1)) (e2 ∷ [])) →
             [_,_]′ bust
                    (λ derivative → valid ((Ap (Lambda List.[ n1 ] derivative)
                                               [ e2 ])))
      ...
      freeDerivativesFromPoints =
        λ { (Ap (Derivative n) (x ∷ [])) →
            Data.Maybe.map (inj₂ ∘ Ap (Derivative n) ∘ [_] ∘ uncurry Ap)
                           (pfRewrite x)
          ; _ → nothing}
      ...
      basicDerivativeChainRuleFunction = b
        ...
        ... | yes fArity1 =
          valid (Ap Product
                    ...
                    ∷ [ Ap fDiff [ Ap g x2 ] ]))
            where
            fDiff = Lambda List.[ n1 ] (Ap (Derivative n1) [ Ap f x ])
...
Basically, some value of type \AgdaDatatype{LT} \AgdaBound{x} \AgdaBound{y} exists if and only if \AgdaBound{x} can be trivially, i.e., without using evaluation steps, shown to be less than \AgdaBound{y}.

\begin{code}
    data LT : ε → ε → Set where
      LT-Rational : {x y : ℚ} → x ℚ.< y → LT (NumberRat x) (NumberRat y)
\end{code}
...
CasanovaFly-Verified = record
  ...
  invae name1 name2 eq = 0 , (inj₁ (cong Variable eq) , sym caeEquality)
    where
    ...
    caeEquality = begin
      main 0 v1 v2
        ≡⟨⟩
      with-dec 0 (structuralEqualityOnε v1 v2)
        ≡⟨ cong (with-dec 0)
                (proj₂ decyes) ⟩
      with-dec 0 (yes (proj₁ decyes))
        ≡⟨⟩
      inj₂ (just (inj₁ (proj₁ decyes)))
        ≡⟨ cong (inj₂ ∘ just ∘ inj₁)
                (≡-irrelevant (proj₁ decyes) (cong Variable eq)) ⟩
      inj₂ (just (inj₁ (cong Variable eq))) ∎
      where
      decyes = dec-yes (structuralEqualityOnε v1 v2) (cong Variable eq)
\end{code}
\end{document}
