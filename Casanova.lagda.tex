...
\begin{code}
  main : {a b eqLevel orderLevel : Level} →
         {A : Set a} →
         {B : Set b} →
         {orderRelation : B → B → Set orderLevel} →
         {eqRelation : B → B → Set eqLevel} →
         IsDecTotalOrder eqRelation orderRelation →
         (A → B) →
...
\begin{code}
  module Properties {a b eqLevel orderLevel : Level}
                    ...
                    {orderRelation : B → B → Set orderLevel}
                    {eqRelation : B → B → Set eqLevel}
                    (totalOrder : IsDecTotalOrder eqRelation orderRelation)
                    (f : A → B) where
\end{code}
...
\begin{code}
    is-ordered :
      ...
      i1 Data.Fin.≤ i2 →
      (orderRelation on (f ∘ List.lookup (main totalOrder f list))) i1 i2 ⊎
      (eqRelation on (f ∘ List.lookup (main totalOrder f list))) i1 i2
    is-ordered = {!!}
\end{code}
...
\begin{code}
  ...
  module lightning where
    ...
    main : Tree (Maybe (Exceptional ε)) →
           List.List (Maybe (Exceptional ε))
    main x = to-witness {m = List.head (sortOn Data.Nat.Properties.≤-isDecTotalOrder
                                               listBadness
                                               (Tree.decompose x))}
                        {!!}
