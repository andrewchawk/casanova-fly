...
\begin{itemize}
 ...
 \item a proof which indicates that if \(n\) does not appear in \(E\), then the \(n\)-derivative of \(E\) is equal to \(E\), and
...
\begin{code}
  AE (Ap Sum (x1 ∷ Ap Sum (x2 ∷ x3 ∷ []) ∷ []))
     ...
     _×_ (AllList.All (λ x → AnyList.Any (AE1 x) ys) xs)
         (AllList.All (λ y → AnyList.Any (AE1 y) xs) ys)
     where
...
\begin{code}
  ...
  AE (Ap Product (x1 ∷ y1 ∷ [])) (Ap Product (x2 ∷ y2 ∷ [])) =
    (AE1 x1 x2 × AE1 y1 y2) ⊎ (AE1 x1 y2 × AE1 y2 y1)
\end{code}
...
module exceptionallyEvaluate where
  ...
  functions =
    ...
    ∷ (λ { (Ap Exponent (x ∷ NumberNatP 0 c ∷ [])) → valid (NumberNat 1)
         ; _ → nothing})
    -- The following "0^x" test case, like the "x^0" test case, is
    -- kind of dubious.  This case, too, mirrors the behavior of
    -- Maxima, though.
