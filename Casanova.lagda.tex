...
\begin{code}
  ...
  module treeEvaluate where
    ...
    main (ℕ.suc n) e =
      treeMap (_>>= [_,_] (const nothing) tryRules) (main n e)
...
\begin{code}
  ...
  module lightning where
    ...
    treeBadness : Tree (Maybe (Exceptional ε)) → ℕ
    treeBadness (Node n) = badness n
    treeBadness (NodeWithChildren n x) = {!!}

    bestValueTree : List.List (Tree (Maybe (Exceptional ε))) →
                    List.List (Maybe (Exceptional ε))
    bestValueTree list = contents (Data.Maybe.to-witness isJust)
      where
      isJust :
        (Is-just ∘ List.head)
          (sortOn Data.Nat.Properties.≤-isDecTotalOrder treeBadness list)
      isJust = {!!}
      contents : Tree (Maybe (Exceptional ε)) → List.List (Maybe (Exceptional ε))
      contents (Node n) = List.[ n ]
      contents (NodeWithChildren n x) = n List.∷ bestValueTree x

    main : Tree (Maybe (Exceptional ε)) →
           List.List (Maybe (Exceptional ε))
    main = bestValueTree ∘ List.[_]
