...
CasanovaFly-Verified = record
  ...
  equality-is-symmetric i {e1} {e2} =
    cartesian-product-predicate-flip sym (eval e1) (eval e2)
    where
    ...
    cartesian-product-predicate-flip = {!!}
    -- In short, the same pair -- well, flipped -- exists in both
    -- cartesian products, so both cartesian products have relevant
    -- Any proofs.
  ...
  invae n1 n2 refl = 0 , _ , sym theProof
    ...
    theProof = begin
      ...
      inj₂ (decToMaybe (any? (uncurry structuralEquality) _))
        ≡⟨ cong (inj₂ ∘ decToMaybe)
                (proj₂ (dec-yes question (AnyList.here refl))) ⟩
      inj₂ (decToMaybe (yes (proj₁ (dec-yes question (AnyList.here refl)))))
        ≡⟨⟩
      inj₂ (just (proj₁ (dec-yes question (AnyList.here refl)))) ∎
      where
      open DefinitelyEqualsInternal
      question = any? (uncurry structuralEquality) _
      v1 = Variable n1
      trace = traceEvaluate 0
\end{code}
\end{document}
