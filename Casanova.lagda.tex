...
record CAS {a : Level} (ε : Set a) : Set (Level.suc Level.zero Level.⊔ a) where
  ...
  module EqualityVerificationParts where
    ...
    evaluationPairs i x y = inj2s (List.cartesianProduct (trace x) (trace y))
      where
...
CasanovaFly-Verified = record
  ...
  where
  equality = CAS._≈_ CasanovaFly-Base
  ...
  equality-is-reflexive i {e} = existence-implies-anyness (proj₂ existence)
    ...
    existence : Σ (Fin (List.length pairs)) (\ i -> uncurry _≡_ (List.lookup pairs i))
    ...
    existence-implies-anyness :
      ...
      P (List.lookup xs i) ->
      AnyList.Any P xs
    existence-implies-anyness {xs = x List.∷ xs} {Fin.zero} p = AnyList.here p
    existence-implies-anyness {xs = x List.∷ xs} {Fin.suc i} p =
      AnyList.there (existence-implies-anyness p)
\end{code}
\end{document}
