...
\begin{code}
  ...
  module lightning where
    complexity : Maybe (Exceptional ε) → ℕ
    complexity nothing = 100000000000000000000000000000 -- This value is not actually complex
                                                        -- but should never occur.
    complexity (just (inj₁ errorCode)) = complexity nothing Data.Nat./ 10
    complexity (just (inj₂ (NumberRat r))) = 0
    complexity (just (inj₂ (Variable name))) = 0
    complexity (just (inj₂ Infinity)) = 0
    complexity (just (inj₂ (Ap f x))) = {!!}

    listComplexity : List.List (Maybe (Exceptional ε)) → ℕ
    listComplexity (x List.∷ List.[]) = complexity x
    listComplexity List.[] = complexity nothing -- Something is incredibly broken.
