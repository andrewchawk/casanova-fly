...
\begin{itemize}
 ...
 \item a proof of the failure of garbage variable substitution\footnote{See \fref{fn:garbageVariable}.}, and
...
\subsection{Using \AgdaDatatype{⊥} for \AgdaInductiveConstructor{Infinity}-Related Types}
...
LessThan : ε → ε → Set
LessThan _ Infinity = ⊥
LessThan Infinity (Variable x) = ⊥
LessThan Infinity (NumberRat x) = ⊥
LessThan Infinity (Ap f x) = ⊥
LessThan (Variable x) (Variable y) = ⊥
LessThan (Variable x) (NumberRat y) = ⊥
LessThan (Variable x) (Ap f y) = ⊥
LessThan (NumberRat x) (Variable y) = ⊥
LessThan (NumberRat x) (NumberRat y) = x ℚ.< y
LessThan (NumberRat x) (Ap f y) = {!!}
LessThan (Ap f x) (Variable y) = ⊥
LessThan (Ap f x) (NumberRat y) = {!!}
LessThan (Ap f x) (Ap g y) = {!!}
\end{code}
...
CasanovaFly-Verified = record
  ...
  invert-inverse e = 1 , inv-inv-propEq e , {!!}
    ...
    inv-inv-propEq e = inj₂ (inj₂ ((0 , 1) ,  inj₁ refl))
