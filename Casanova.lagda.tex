...
\begin{enumerate}
  ...
  \item \(b\) immediately computes simple integrals, e.g., \(\int_{x=0}^5 \sin x\).  The result is represented directly as a floating-point number, integer, or whatever type happens to be the best fit.  In any case, the immediate result \emph{cannot be further simplified}.
\end{enumerate}
