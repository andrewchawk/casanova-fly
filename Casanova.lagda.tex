...
\subsection{The Single-Step Evaluation Function}
\AgdaFunction{exceptionallyEvaluate} \AgdaBound{x} contains the result of performing a single evaluation step on \AgdaBound{x} \emph{or} a value which indicate that \AgdaBound{x} is in some way nonsensical.
...
Basically, according to \AgdaInductiveConstructor{Symmetry}, if \(a = b\) and \(b = c\), then \(a = c\).
