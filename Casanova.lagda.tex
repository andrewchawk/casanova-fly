...
\AgdaFunction{uniquePrepend} indicates that if \AgdaBound{x} is not in \AgdaBound{xs} and \AgdaBound{alls}, then \AgdaFunction{pos} \AgdaSymbol(\AgdaBound{x} \AgdaOperator{\AgdaFunction{List.∷}} \AgdaBound{xs}\AgdaSymbol) contains the successor of \AgdaFunction{pos} \AgdaBound{xs} or is \AgdaInductiveConstructor{nothing}.
