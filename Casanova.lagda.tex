...
\AgdaFunction{nonRepeatingPortion} \AgdaBound{DEQ} \AgdaBound{x} is the longest initial non-repeating subsequence within \AgdaBound{x}.
...
nonRepeatingPortion : {a : Level} ->
                      ...
                      List.List A ->
                      List.List A
nonRepeatingPortion {_} {A} DEQ = helper List.[]
  where
  helper : List.List A -> List.List A -> List.List A
  helper List.[] (x List.∷ xs) = helper List.[ x ] xs
  helper as List.[] = as
  helper as (x List.∷ xs) = with-any (any? (DEQ x) as)
    where
    with-any : Dec (AnyList.Any (\ a -> x ≡ a) as) -> List.List A
    with-any (yes listRepeats) = as
    with-any (no addToList) = helper (as List.++ List.[ x ]) xs
\end{code}
...
A value of type \AgdaField{CAS.\AgdaUnderscore{}≈\AgdaUnderscore{}} \AgdaBound{c} \AgdaBound{i} \AgdaBound{x} \AgdaBound{y} exists if and only if \AgdaBound{x} is shown to be algebraically equal to \AgdaBound{y} within \AgdaBound{i} evaluation steps.
...
\AgdaField{CAS.simpleLimitRepresentation} \AgdaBound{c} \AgdaBound{x} \AgdaBound{c} \AgdaBound{e} is \AgdaBound{c}'s representation of \(\lim_{\AgdaBound{c} \rightarrow \AgdaBound{x}} \textrm{\Epsilon}\), where ``\(\textrm{\Epsilon}\)'' denotes the expanded version of the \AgdaBound{e} expression.
...
record CAS {a : Level} (ε : Set a) : Set (Level.suc Level.zero Level.⊔ a) where
  ...
  traceEvaluate n e =
    nonRepeatingPortion (Data.Sum.Properties.≡-dec Data.String._≟_ structuralEquality)
                        (List.map (flip rEvaluate e) (List.upTo n))

  _≈_ : ℕ -> ε -> ε -> Set a
  _≈_ iterations x y = AnyList.Any (uncurry _≡_) evaluationPairs
    where
    evaluationPairs = inj2s (List.cartesianProduct (trace x) (trace y))
      where
  ...
  bogus-at-some-iteration e = Σ ℕ (\ n -> bogus-at-iteration n e)
...
\begin{itemize}
 ...
 \item a proof of commutativity of multiplication, and
...
\AgdaField{limit-of-unrelated-value-is-unrelated-value} \AgdaBound{v} indicates that in the \AgdaBound{v} CAS, if \(v\) is not used in \(e\), then \(\lim_{v \rightarrow b} e\) is equivalent to \(e\) for all \(b\), \(e\), and \(v\).
...
commutativeEvaluate e@(Ap f x) =
  ...
  tryFlip (yes arityMatches) =
    if commutes ∧ (e2 evaluates) then inj₂ e2 else evaluate e
    where
    commutes = isCommutative f arityMatches
    e2 = Ap f (reverse x)
