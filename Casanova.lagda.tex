...
Any two given variables are not necessarily equal.  Accordingly, \AgdaField{CAS.differently-named-variables-are-not-definitely-equal} \AgdaBound{c} indicates that within \AgdaBound{c}, given any two \AgdaBound{c} variables \(x\) and \(y\), if the name of \(x\) differs from the name of \(y\), then \(x\) is not definitely equal to \(y\).

\subsection{The Proof which Indicates that Variables of Identical Names are Algebraically Equal}
Also, options on variables are only sensible if for all variables \(x\) and \(y\), if the name of \(x\) is also the name of \(y\), then \(x\) is algebraically equal to \(y\).  Accordingly, \AgdaField{CAS.identically-named-variables-are-equal} \AgdaBound{c} indicates that for any \AgdaBound{c} variables \(x\) and \(y\), if the name of \(x\) is identical to the name of \(y\), then \AgdaBound{c} can determine that \(x\) is algebraically equal to \(y\).
...
\begin{itemize}
 \item a proof which indicates that any non-bogus element \(e\) of type \AgdaBound{ε} is definitely equal to \(e\),
 \item a proof which indicates that the algebraic equality relation is symmetric,
 \item a proof which indicates that the algebraic equality relation is transitive,
...
\subsection{The Equality Symmetry Field}
A good CAS handles complex numbers, on which equality is symetric.  Accordingly, a verified CAS should have a \emph{symmetric} algebraic equality relation.  \AgdaField{VCAS.symmetry-of-equality} \AgdaBound{V} proves that the algebraic equality relation for the \AgdaBound{V} CAS is symmetric.

\subsection{The Equality Transitivity Field}
A good CAS handles complex numbers, on which equality is \emph{also} transitive.  Accordingly, a verified CAS should have a \emph{transitive} algebraic equality relation.  \AgdaField{VCAS.transitivity-of-equality} \AgdaBound{V} proves that the algebraic equality relation for the \AgdaBound{V} CAS is transitive.
...
record VCAS {a : Level} (ε : Set a) (Cas : CAS ε) : Set a where
  ...
  field
    reflexivity : (e : ε) ->
                  ...
                  Σ (e ≈ e)
                    (\ equality -> inj₂ (just equality) ≡ definitelyEquals e e)
    symmetry-of-equality : Symmetric _≈_
    transitivity-of-equality : Transitive _≈_
