...
\begin{code}
{-# TERMINATING #-}
exceptionallyEvaluate = λ e → firstInj2 $ Data.Vec.map (_$ e) functions
  -- The preceding lambda expression prevents unnecessarily binding "e".
  where
  firstInj2 : {n : ℕ} → {a : Level} → {A : Set a} →
              Vec (Exceptional A) (ℕ.suc n) → Exceptional A
  firstInj2 (inj₁ x ∷ []) = inj₁ x
  firstInj2 (inj₂ x ∷ xs) = inj₂ x
  firstInj2 (inj₁ x ∷ xs@(xs1 ∷ xs2)) = firstInj2 xs

  functions =
    ( (λ { (Ap Negate (Ap Negate (x ∷ []) ∷ [])) → inj₂ x
         ; _ → break "Double negation"})
    ∷ (λ { (Ap Negate (NumberRat x ∷ [])) → inj₂ (NumberRat (ℚ.0ℚ ℚ.- x))
         ; _ → break "Rational negation"})
    ∷ (λ { (Ap Sum (NumberRat x ∷ NumberRat y ∷ [])) →
           inj₂ (NumberRat (x ℚ.+ y))
         ; _ → break "Rational addition"})
    ∷ (λ { (Ap Product (NumberRat x ∷ NumberRat y ∷ [])) →
           inj₂ (NumberRat (x ℚ.* y))
         ; _ → break "Rational multiplication"})
    ∷ (λ { (Ap Exponent (NumberRat x ∷ NumberRat y ∷ [])) →
           if ((ℚ.denominatorℕ x Data.Nat.≡ᵇ 1) ∧
              ...
              then (let numerator = Data.Integer.∣_∣ ∘ ℚ.numerator in
                    if (numerator x Data.Nat.≡ᵇ 0) ∧
                       (numerator y Data.Nat.≡ᵇ 0)
                     then inj₁ "0^0 is undefined!"
                     else inj₂ (NumberNat (numerator x Data.Nat.^ numerator y)))
              else break "Rational exponentiation"
         ; _ → break "Rational exponentiation"})
    ∷ (λ { (Ap (Lambda names body) expressions) →
           inj₂ (varSubstM (List.zip names (Data.Vec.toList expressions)) body)
         ; _ → break "Lambda substitution"})
    ∷ (λ { (Ap f x) → [_,_] inj₁
                            (inj₂ ∘ Ap f)
                            (condense (Data.Vec.map exceptionallyEvaluate x))
         ; _ → break "The function application fallback"})
    ∷ [ inj₂ ]
    )
    where
    break : String → Exceptional ε
    break s = inj₁ (s Data.String.++ " is not applicable here.")
    condense : {a : Level} →
               {A : Set a} →
               {n : ℕ} →
               Vec (Exceptional A) n →
