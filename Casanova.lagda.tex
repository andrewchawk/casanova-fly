...
\begin{itemize}
  \item \AgdaFunction{repeatExceptional} \AgdaNumber{0} \AgdaBound{f} \AgdaBound{x} is \AgdaBound{f} \AgdaBound{x}.
  \item If \AgdaBound{f} \AgdaBound{x} is some \AgdaInductiveConstructor{inj₁} value, then then \AgdaFunction{repeatExceptional} \AgdaBound{n} \AgdaBound{f} \AgdaBound{x} is \AgdaBound{f} \AgdaBound{x}.
  \item If \AgdaBound{f} \AgdaBound{x} is some value \AgdaInductiveConstructor{inj₂} \AgdaBound{x'}, then \AgdaFunction{repeatExceptional} \AgdaSymbol(\AgdaInductiveConstructor{ℕ.suc} \AgdaBound{n}\AgdaSymbol) \AgdaBound{f} \AgdaBound{x} is equivalent to \AgdaFunction{repeatExceptional} \AgdaBound{n} \AgdaBound{f} \AgdaBound{x'}.
\end{itemize}
