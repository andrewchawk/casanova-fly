...
isCommutative (Compose g f eq) equality = false
\end{code}
...
If the result is not \AgdaInductiveConstructor{nothing}, then \AgdaFunction{pfRewrite} \AgdaBound{e} contains values \AgdaBound{g} and \AgdaSymbol(\AgdaBound{len} \AgdaOperator{\AgdaInductiveConstructor{,}} \AgdaBound{x}\AgdaSymbol) such that the following statements hold:
