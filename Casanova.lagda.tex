...
\begin{enumerate}
  \item \AgdaFunction{repeatExceptional} \AgdaNumber{0} \AgdaBound{f} \AgdaBound{x} is \AgdaBound{f} \AgdaBound{x}.\label{enum:zeroIsBasicApplication}
  \item If \AgdaBound{f} \AgdaBound{x} is some \AgdaInductiveConstructor{inj₁} value, then then \AgdaFunction{repeatExceptional} \AgdaBound{n} \AgdaBound{f} \AgdaBound{x} is \AgdaBound{f} \AgdaBound{x}.\label{enum:inj1GetsReturned}
...
\AgdaFunction{zero-is-basic-application}, indicates that in accordance with \fref{enum:zeroIsBasicApplication}, \AgdaFunction{repeatExceptional} \AgdaNumber{0} \AgdaBound{f} \AgdaBound{x} is just \AgdaBound{f} \AgdaBound{x}.
...
In accordance with \fref{enum:inj1GetsReturned}, \AgdaFunction{inj₁-immediate-return} indicates that if \AgdaBound{f} \AgdaBound{x} is some \AgdaInductiveConstructor{inj₁} value, then \AgdaFunction{repeatExceptional} \AgdaBound{n} \AgdaBound{f} \AgdaBound{x} is always \AgdaBound{f} \AgdaBound{x}.
...
\begin{enumerate}
  ...
  \item If \AgdaBound{x} and \AgdaBound{y} are non-bogus for all iteration counts which are less than or equal to \AgdaBound{n}, but no appropriate equality proof exists, then \AgdaFunction{checkAlgebraicEquality} \AgdaBound{n} \AgdaBound{x} \AgdaBound{y} is \AgdaInductiveConstructor{inj₂} \AgdaInductiveConstructor{nothing}.
  \item If \AgdaBound{x} or \AgdaBound{y} is bogusly evaluated at some iteration count which is less than or equal to \AgdaBound{n}, then \AgdaFunction{checkAlgebraicEquality} \AgdaBound{n} \AgdaBound{x} \AgdaBound{y} is \AgdaInductiveConstructor{inj₁} \AgdaBound{s}, where \AgdaBound{s} is some description of the aforementioned bogosity.
\end{enumerate}
