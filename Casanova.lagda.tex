...
\subsection{The Subtree Relation}
...
\subsection{The Decomposition Function}
...
\subsection{Lookup Functions and Associated Types}

\subsection{The Location Type}\label{sec:TreeLocationDefinition}
...
\subsection{The Lookup Function}\label{sec:lookupTreeDefinition}
...
\subsection{Properties of Trees and Related Functions}
...
\subsubsection{The Proof of the Non-Zero Length of Decompositions}
...
\begin{code}
    ...
    decompose-length-is-greater-than-zero (NodeWithChildren n x) =
      {!!}
\end{code}

\subsubsection{The Proof of \AgdaFunction{treeMap}'s Generation of Supertrees}
...
\subsection{Exporting Particularly Useful Functions and Such}
...
\begin{code}
  main : {a b eqLevel orderLevel : Level} →
         {A : Set a} →
         (totalOrder : DecTotalOrder b eqLevel orderLevel) →
         (A → DecTotalOrder.Carrier totalOrder) →
         (List.List A → List.List A)
  main total f = ListMergeSort.sort total2
    where
    total2 = record
      { Carrier = _
      ; _≈_ = DecTotalOrder._≈_ total on f
      ; _≤_ = DecTotalOrder._≤_ total on f
      ; isDecTotalOrder = {!!}
      }
\end{code}
...
\begin{code}
  module Properties {a b eqLevel orderLevel : Level}
                    {A : Set a}
                    (totalOrder : DecTotalOrder b eqLevel orderLevel)
...
\begin{code}
    is-ordered :
      (list : List.List A) →
      (i1 i2 : Fin (List.length (main totalOrder f list))) →
      i1 Data.Fin.≤ i2 →
      (DecTotalOrder._≤_ totalOrder on (f ∘ List.lookup (main totalOrder f list))) i1 i2 ⊎
      (DecTotalOrder._≈_ totalOrder on (f ∘ List.lookup (main totalOrder f list))) i1 i2
    is-ordered = {!!}
\end{code}
...
\begin{code}
    same-length : (list : List.List A) →
                  List.length list ≡ List.length (main totalOrder f list)
    same-length = {!!}
\end{code}
...
\begin{code}
    generates-permutation : (list : List.List A) → list ↭ main totalOrder f list
...
\begin{code}
  treeEvaluate : ℕ → ε → Tree (Maybe (Exceptional ε))

  module treeEvaluate where
    tryRules : ε → Maybe (List.List (Tree (Maybe (Exceptional ε))))
    ...
    ... | List.[] = nothing
    ... | o@(x List.∷ xs) = just (List.map (Node ∘ just) o)

    main : ℕ → ε → Tree (Maybe (Exceptional ε))
    main 0 e = Node (just (inj₂ e))
    main (ℕ.suc n) e = treeMap (_>>= [_,_] (const nothing) tryRules) (main n e)
...
\AgdaFunction{lightning} is so named because the overall functionality reminds the author of the path-of-least-resistance behavior which is exhibited by lightning upon the hitting of a tree; the electricity traces the most direct path to the ground from the hit branch --- or leaf ---, much like \AgdaFunction{lightning} traces the most direct path from the root node, i.e., the expression which has been evaluated, to the most suitable leaf node, i.e., the result of the evaluation.

\begin{code}
  lightning : Tree (Maybe (Exceptional ε)) →
              List.List (Maybe (Exceptional ε))

  module lightning where
    badness : Maybe (Exceptional ε) → ℕ
    badness nothing = 100000000000000000000000000000 -- Something is busted.
    badness (just (inj₁ errorCode)) = badness nothing Data.Nat./ 10
    badness (just (inj₂ (NumberRat r))) = 0
    badness (just (inj₂ (Variable name))) = 0
    badness (just (inj₂ Infinity)) = 0
    badness (just (inj₂ (Ap f x))) = {!!}

    listBadness : List.List (Maybe (Exceptional ε)) → ℕ
    listBadness (x List.∷ List.[]) = badness x
    listBadness List.[] = 10000000000000000000000000 -- Something is incredibly broken.
    listBadness (x List.∷ xs) = ℕ.suc (listBadness xs)

    main : Tree (Maybe (Exceptional ε)) →
           List.List (Maybe (Exceptional ε))
