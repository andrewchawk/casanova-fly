...
\begin{itemize}
 ...
 \item a proof which indicates that the derivative of a constant is \(0\), and
...
isCommutative (Diff f) equality = false
isCommutative (Compose g f) equality = false
\end{code}
...
structuralEqualityOnF = s
  ...
  s (Lambda n1 e1) (Compose g f) = no λ ()
  ...
  s (Derivative n1) (Compose g f) = no λ ()
  ...
  s (Antiderivative n1) (Compose g f) = no λ ()
  ...
  s (BoundedIntegral n1) (Compose g f) = no λ ()
  s (Diff f) Abs = no λ ()
  ...
  s (Diff f) (Compose y x) = no λ ()
  s (Compose g f) Abs = no λ ()
  s (Compose g f) Negate = no λ ()
  s (Compose g f) Sum = no λ ()
  s (Compose g f) Product = no λ ()
  s (Compose g f) Exponent = no λ ()
  s (Compose g f) (Lambda name exp) = no λ ()
  s (Compose g f) Sin = no λ ()
  s (Compose g f) Cos = no λ ()
  s (Compose g f) Tan = no λ ()
  s (Compose g f) Csc = no λ ()
  s (Compose g f) Sec = no λ ()
  s (Compose g f) Cot = no λ ()
  s (Compose g f) (Limit name) = no λ ()
  s (Compose g f) (Derivative name) = no λ ()
  s (Compose g f) (Antiderivative name) = no λ ()
  s (Compose g f) (BoundedIntegral name) = no λ ()
  s (Compose g f) (Diff x) = no λ ()
  s (Compose g f) (Compose y x) with s g y | s f x
  ... | yes gIsy | yes fIsx = yes (cong₂ Compose gIsy fIsx)
  ... | no gIsNoty | _ = no (gIsNoty ∘ eq-implies-first-eq)
    where
      eq-implies-first-eq :
        {f g x y : F} → Compose g f ≡ Compose y x → g ≡ y
      eq-implies-first-eq refl = refl
  ... | _ | no fIsNotx = no (fIsNotx ∘ eq-implies-second-eq)
    where
    eq-implies-second-eq :
      {f g x y : F} → Compose g f ≡ Compose y x → f ≡ x
...
module exceptionallyEvaluate where
  ...
  functions =
    ...
    basicDerivativeChainRuleFunction = b
      ...
      b (Ap (Derivative n1) (Ap f x ∷ [])) with arity f | x | inspect arity f
      ... | 1 | (Ap g y ∷ []) | ⟦ arityOfFIs1 ⟧ = with-arity (arity g Data.Nat.≟ 1)
        where
        with-arity : Dec (arity g ≡ 1) → Maybe (Exceptional ε)
        with-arity (no nonApplicable) = nothing
        with-arity (yes eq) =
          valid (Ap Product
                    ( Ap (Derivative n1) [ Ap g y ]
                    ∷ [ Ap fDiff [ Ap g y ] ]))
          where
          g-f-arity-equality = trans eq (sym arityOfFIs1)
          fDiff = Lambda List.[ n1 ]
                         (Ap (Derivative n1)
                             [ Ap f (Data.Vec.cast g-f-arity-equality y) ])
      ... | _ | _ | _ = nothing
      b _ = nothing
