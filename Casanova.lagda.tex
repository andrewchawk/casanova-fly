...
\subsection{The Properties of the Function}
This module contains a few proofs which indicates that \AgdaFunction{firstRepeatPosition} really does work as described, after all.
...
\subsubsection{The Proof of \AgdaFunction{firstRepeatPositionHelper}'s Buffer Reversal}
Basically, \AgdaFunction{posh-moves-elements} proves that the order of the elements in \AgdaFunction{firstRepeatPositionHelper} is the opposite of the original list's order.
...
Just a bit informally, one can say that \AgdaFunction{pos-is-posh} proves that if \AgdaBound{x} is not in \AgdaBound{xs}, then \AgdaFunction{pos} \AgdaBound{xs} is equivalent to \AgdaFunction{posh} \AgdaOperator{\AgdaFunction{List.[}} \AgdaBound{x} \AgdaOperator{\AgdaFunction{]}} \AgdaBound{xs}.
...
With the exception of the final ``\AgdaKeyword{open}'' bit, this section constitutes a module.
...
\AgdaDatatype{Tree} is a basic tree datatype.
...
A value of type \AgdaDatatype{IsSubtree} \AgdaBound{t1} \AgdaBound{t2} exists if and only if \AgdaBound{t1} is a subtree of \AgdaBound{t2}.
...
A bit informally, one can say that \AgdaFunction{decompose} \AgdaBound{tree} is the list of paths from the root node of \AgdaBound{tree} to all terminal nodes of \AgdaBound{tree}.
...
For every \AgdaInductiveConstructor{Node} \AgdaBound{n} in \AgdaBound{x}, the following statements apply:

\begin{enumerate}
  \item If \AgdaBound{j} is some value of type \AgdaFunction{Is-just} \AgdaSymbol(\AgdaBound{f} \AgdaBound{n}\AgdaSymbol), then \AgdaInductiveConstructor{NodeWithChildren} \AgdaBound{n} \AgdaSymbol(\AgdaFunction{to-witness} \AgdaBound{j}\AgdaSymbol) replaces \AgdaInductiveConstructor{Node} \AgdaBound{n} in \AgdaFunction{treeMap} \AgdaBound{f} \AgdaBound{x}.\label{enum:treeMap-just}
  \item If \AgdaBound{f} \AgdaBound{n} is \AgdaInductiveConstructor{nothing}, then \AgdaInductiveConstructor{Node} \AgdaBound{n} is not changed in \AgdaFunction{treeMap} \AgdaBound{f} \AgdaBound{x}.\label{enum:treeMap-nothing}
\end{enumerate}
...
A value of type \AgdaFunction{TreeLocation} \AgdaBound{t} represents the location of a subtree of \AgdaBound{t}.  Equivalently but possibly a bit more intuitively, one can say that a value of type \AgdaFunction{TreeLocation} \AgdaBound{t} represents a series of instructions for navigating to a particular subtree of \AgdaBound{t}.

The author finds that \AgdaFunction{TreeLocation} is best understood in conjunction with \AgdaFunction{lookupTree}, which is defined in \fref{sec:lookupTreeDefinition}.
...
\AgdaFunction{lookupTree} \AgdaBound{t} \AgdaBound{location} is the subtree of \AgdaBound{t} to which \AgdaBound{location} points.

The author thinks that \AgdaFunction{lookupTree} is best understood in conjunction with \AgdaFunction{TreeLocation}, which is defined in \fref{sec:TreeLocationDefinition}.
...
This subsection constitutes a module.  More specifically, this subsection constitutes a module regarding the properties of things which are related to \AgdaDatatype{Tree}, e.g., \AgdaFunction{treeMap}.
...
Basically, \AgdaFunction{decompose-length-is-greater-than-zero} proves that the decomposition of any \AgdaDatatype{Tree} is of a length which is greater than zero.
...
\subsection{The Proof of \AgdaFunction{treeMap}'s Generation of Supertrees}
Just a bit informally, one can say that \AgdaFunction{treeMap-creates-supertree} proves that \AgdaBound{t} is a subtree of \AgdaFunction{treeMap} \AgdaBound{f} \AgdaBound{t} for all relevant \AgdaBound{f} and \AgdaBound{t}.
...
\subsubsection{The \AgdaInductiveConstructor{Node} Lookup Proof}
Basically, \AgdaFunction{node-lookup-is-node} proves that \AgdaFunction{lookupTree} \AgdaSymbol{(}\AgdaInductiveConstructor{Node} \AgdaBound{n}\AgdaSymbol{)} \AgdaBound{l} is \AgdaInductiveConstructor{Node} \AgdaBound{n} for all appropriate \AgdaBound{n} and \AgdaBound{l}.

Determining the origins of ``\AgdaBound{n}'' and ``\AgdaBound{l}'' is left as an exercise for readers who might be entertained by shaken keys.

\begin{code}
    node-lookup-is-node :
      {a : Level} →
      {A : Set a} →
      ∀ n l → lookupTree {A = A} (Node n) l ≡ Node n
    node-lookup-is-node node location = refl
\end{code}

\subsection{The \AgdaInductiveConstructor{nothing} Lookup Proof}
\AgdaFunction{lookupTree-correctly-handles-nothing} proves that \AgdaFunction{lookupTree} \AgdaBound{t} \AgdaInductiveConstructor{nothing} is \AgdaBound{t} for all relevant \AgdaBound{t}.

\begin{code}
    lookupTree-correctly-handles-nothing :
      {a : Level} →
      {A : Set a} →
      (node : A) →
      (trees : List.List (Tree A)) →
      lookupTree (NodeWithChildren node trees) nothing ≡ NodeWithChildren node trees
    lookupTree-correctly-handles-nothing node trees = refl
\end{code}

\AgdaFunction{lookupTree-correctly-recurses}, as mostly indicated by the name, essentially proves that \AgdaFunction{lookupTree} correctly handles recursion when given a \AgdaInductiveConstructor{just} value.

\begin{code}
    lookupTree-correctly-recurses :
      {a : Level} →
      {A : Set a} →
      ∀ (node : A) treeList location →
      _≡_ (lookupTree (NodeWithChildren node treeList) (just location))
          (lookupTree (List.lookup treeList (proj₁ location)) (proj₂ location))
    lookupTree-correctly-recurses node treeList location = refl
\end{code}

\section{Exporting Particularly Useful Functions and Such}
To prevent the excessive use of the relevant name qualifier, the author has chosen to permit the unqualified use of certain \AgdaDatatype{Tree}-related identifiers.
...
\AgdaFunction{sortOn} is a generalized sorting function.

Informally, one can say that \AgdaFunction{sortOn} \AgdaBound{totalOrder} \AgdaBound{f} \AgdaBound{list} sorts, in ascending order, \AgdaFunction{List.map} \AgdaBound{f} \AgdaBound{list}, replaces the values in this \AgdaFunction{List.map} list with the original \AgdaBound{list} values, and returns the result.

More formally, one can say that \AgdaFunction{sortOn} is such that the following statements apply for all relevant \AgdaBound{i1}, \AgdaBound{i2}, \AgdaBound{list}, and \AgdaBound{f}:

\begin{enumerate}
  \item \(i_1 \leq i_2\) if and only if \AgdaFunction{List.lookup} \AgdaSymbol(\AgdaFunction{sortOn} \AgdaBound{totalOrder} \AgdaBound{list}\AgdaSymbol) \AgdaBound{i1} is less than or equal to \AgdaFunction{List.lookup} \AgdaSymbol(\AgdaFunction{sortOn} \AgdaBound{totalOrder} \AgdaBound{list}\AgdaSymbol) \AgdaBound{i2}, where \(i_1\) and \(i_2\) are the values which are represented by \AgdaBound{i1} and \AgdaBound{i2}, respectively.\label{enum:sortOn-ordering}
  \item \AgdaFunction{sortOn} \AgdaBound{totalOrder} \AgdaBound{list} is a permutation of \AgdaBound{list}.\label{enum:sortOn-permutation}
\end{enumerate}
...
\subsection{The Total Order on \AgdaBound{A}}
\AgdaFunction{newTotalOrder} is the total order on \AgdaBound{A} which is used for sorting.
...
\AgdaFunction{main} will become the actual \AgdaFunction{sortOn} function.
...
\subsection{Properties of the Function}
This submodule contains proofs of the various properties of \AgdaFunction{sortOn}.
...
\AgdaFunction{is-ordered} proves that \fref{enum:sortOn-ordering} is actually relevant.
...
\AgdaFunction{generates-permutation} proves that \fref{enum:sortOn-permutation} correctly describes \AgdaFunction{sortOn}.
...
One can simply say that \AgdaFunction{same-length} proves that sorting a list yields a list whose length is the length of the input list.
...
\subsection{Exporting the Function}
This subsection marks the end of the \AgdaFunction{sortOn} module.  Anyway, to prevent excessive verbosity, the author has defined yet another abbreviated identifier.
...
\AgdaFunction{bifactorNat} is the natural number bifactoring function, i.e., \AgdaFunction{bifactorNat} \AgdaBound{n} is the list of all natural number pairs \(p\) such that the product of the parts of \(p\) is equal to \AgdaBound{n}.
...
\AgdaFunction{possibleFactors} \AgdaBound{n} is a list of all possible factors of \AgdaBound{n}, i.e., natural numbers which are less than or equal to \(\sqrt{n\prime}\), where \(n\prime\) is the number which is represented by \AgdaBound{n}.
...
\AgdaFunction{pairList} \AgdaBound{n} is a list of all possible factor pairs of \AgdaBound{n}, i.e., 2-tuples whose product might be equal to \AgdaBound{n}.
...
\AgdaFunction{main} will eventually become \AgdaFunction{bifactorNat}.
...
A smidge informally, one can say that \AgdaFunction{each-element-is-factor-pair} proves that \AgdaFunction{main} \AgdaBound{n} contains only pairs of elements whose product is equal to \AgdaBound{n}.  A bit more formally, one can say that the function proves that for any \AgdaBound{f1} \AgdaOperator{\AgdaInductiveConstructor{,}} \AgdaBound{f2} in \AgdaFunction{main} \AgdaBound{n}, \(f_1 \cdot f_2 = n\prime\), where \(f_1\), \(f_2\), and \(n\prime\) are the values which are represented by \AgdaBound{f1}, \AgdaBound{f2}, and \AgdaBound{n}, respectively.
...
One can more or less paraphrase the name to say that \AgdaFunction{each-factor-pair-is-in-list} proves that each factor pair for \AgdaBound{n} is an element of \AgdaFunction{main} \AgdaBound{n}.
...
\subsection{Exporting the Function}
As indicated in \fref{sec:bifactorNat-main}, the author has decided to provide the ``\AgdaFunction{bifactorNat}'' alias for \AgdaFunction{bifactorNat.main}.
...
\AgdaFunction{CAS.factor} \AgdaBound{c} \AgdaBound{e} is a list of every value whose product is \AgdaBound{c}-\gls{algEq} to \AgdaBound{e}.
...
\AgdaField{VCAS.limit-of-non-bogus-expression} \AgdaBound{v} proves that if \(f\ y\) is not \gls{bogusWrt} \AgdaBound{c}, then \(\lim_{x \rightarrow y} \left(f\ x\right)\) is \AgdaBound{c}-\gls{algEq} to \(f\ y\).

\subsection{The L'H\^opital's Rule Proof Field}
\AgdaField{VCAS.lHopital} \AgdaBound{v} proves that L'H\^opital's rule applies to the \AgdaBound{v} CAS.
...
\AgdaField{VCAS.factor-input-is-product-of-output} \AgdaBound{v} proves that for every value \(x\) in \AgdaField{CAS.factor} \AgdaBound{c} \AgdaBound{e}, the product of \(x\) is \gls{algEq} to \AgdaBound{e}, where \AgdaBound{v} is a value of type \AgdaRecord{VCAS} \AgdaBound{c}.

\section{The Factor List Containment Proof}
\AgdaField{VCAS.factor-input-is-product-of-output} \AgdaBound{v} proves that if the product of some list \(l\) is \AgdaBound{c}-\gls{algEq} to \AgdaBound{e}, then \(l\) is in \AgdaField{CAS.factor} \AgdaBound{c} \AgdaBound{e}, where \AgdaBound{v} is a value of type \AgdaRecord{VCAS} \AgdaBound{c}.
...
\begin{itemize}
  ...
  \item \AgdaInductiveConstructor{Ap} \AgdaBound{f} \AgdaBound {e} represents the result of applying \AgdaBound{f} to \AgdaBound{e}.  A touch more formally, one can say that \AgdaInductiveConstructor{Ap} \AgdaBound{f} \AgdaBound{x} represents \(\operatorname{uncurry}\ f\prime\ x\prime\), where the following statements apply:
  \begin{enumerate}
    \item \(\operatorname{uncurry}\) is the univeral uncurrying function
    \item \(f\prime\) and \(x\prime\) are the things which are represented by \AgdaBound{f} and \AgdaBound{x}, respectively.
  \end{enumerate}
...
A value of type \AgdaDatatype{main} \AgdaBound{e} exists if and only if \AgdaBound{e} is a constant, i.e., refers to no variables.
...
This section contains proofs of the various properties of \AgdaDatatype{main}.

All parts of \fref{sec:isConstantProperties} which follow the module declaration are in the module.
...
A smidge informally, one can say that \AgdaFunction{constantOnlyIfXConstant} proves that the application of a function is a constant only if all arguments of the function are constants.
...
As the reader may have guessed, the author has decided to define \AgdaFunction{IsConstant} as being equal to \AgdaDatatype{IsConstant.main}.
...
\section{The Tree-Based Evaluation Function}
\AgdaFunction{treeEvaluate} is a function\footnote{You don't repeat.} which, given an iteration bound \AgdaBound{i} and an expression \AgdaBound{e}, returns a tree which represents all paths to all evaluation results which can be achieved within \AgdaBound{i} iterations.

\begin{code}
  treeEvaluate : ℕ → ε → Tree (Exceptional ε)

  module treeEvaluate where
    tryRules : ε → Maybe (List.List (Tree (Exceptional ε)))
    tryRules e with (List.filter (new? (inj₂ e)) ∘ List.catMaybes) (List.map (_$ e) funcs)
      where
      funcs = Data.Vec.toList exceptionallyEvaluate.functions
      new? : (e1 e2 : Exceptional ε) → Dec _
      new? e1 e2 = true Data.Bool.≟ isNo (eq e1 e2)
        where eq = Data.Sum.Properties.≡-dec Data.String._≟_ structuralEqualityOnε
    ... | List.[] = nothing
    ... | o@(x List.∷ xs) = just (List.map Node o)

    main : ℕ → ε → Tree (Exceptional ε)
    main 0 e = Node (inj₂ e)
    main (ℕ.suc n) e = treeMap ([_,_] (const nothing) tryRules) (main n e)

  treeEvaluate = treeEvaluate.main
\end{code}

\subsection{The Lightning Function}
\AgdaFunction{lightning} returns a single path to a single evaluated form.

\AgdaFunction{lightning} is so named because the overall functionality reminds the author of the path-of-least-resistance behavior which is exhibited by lightning upon contact with a tree; the electricity traces the most direct path to the ground from the hit branch --- or leaf ---, much like \AgdaFunction{lightning} traces the most direct path from the root node, i.e., the expression which has been evaluated, to the most suitable leaf node, i.e., the result of the evaluation.
...
\newglossaryentry{algEq}
  ...
  , description = Any appropriate thing \(x\) is \AgdaBound{c}-\gls{algEq} to any appropriate thing \(y\) if and only if \(x\) is \AgdaBound{c}-\gls{algEq1} or \AgdaBound{c}-\gls{algEq2} to \(y\).  This term is vaguer than ``\AgdaBound{c}-\gls{algEq2}'' and ``\AgdaBound{c}-\gls{algEq1}'' and facilitates more concise communication
  }
